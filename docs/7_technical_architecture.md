...
**Strategy**: Hybrid approach untuk maximum speed + developer experience

**1. Core: TailwindCSS + Custom Components**
- Simple components (Button, Input, Card) = **custom built** dengan pure Tailwind
- Zero runtime JS overhead
- Full control over bundle size
- Total: ~5KB for basic components

**2. Complex Interactions: shadcn/ui (SELECTIVE)**
**Why shadcn?**
- ✅ Copy-paste into codebase (not npm dependency = no bloat)
- ✅ Built on Radix UI (accessible, headless)
- ✅ Customizable dengan Tailwind
- ✅ Tree-shakeable (ambil yang dipake aja)

**ONLY use shadcn for**:
- ✅ Dialog/Modal (accessibility complex)
- ✅ Dropdown Menu (keyboard navigation)
- ✅ Tooltip (positioning logic)
- ✅ Tabs (ARIA roles)

**DON'T use shadcn for**:
- ❌ Button (too simple, Tailwind cukup)
- ❌ Input (custom faster)
- ❌ Card (pure CSS)

**3. Headless Primitives: Radix UI**
For components NOT in shadcn but need accessibility:
- `@radix-ui/react-toast` (notifications)
- `@radix-ui/react-progress` (loading bar)

**Bundle Size Target**:
- TailwindCSS: ~10KB
- Custom components: ~5KB
- shadcn components (4-5 selected): ~8KB
- Radix primitives: ~6KB
- **Total UI bundle: <30KB** 🎯

**Performance Guidelines**:
1. **Island Architecture**: Only load interactive components on client
   ```astro
   <Dialog client:visible /> <!-- Load only when visible -->
   ```
2. **No Animation Libraries**: Use CSS transforms (GPU-accelerated)
3. **Lazy Load**: Heavy components (share modal) load on demand
4. **SVG Icons**: Inline, not icon library (lucide-react = 50KB!)

**Comparison**:
| Library | Bundle Size | Speed | DX | Verdict |
|---------|-------------|-------|----|----|
| **Our Stack** | ~30KB | ⚡⚡⚡ | ⭐⭐⭐ | ✅ Best |
| Material UI | ~200KB | 🐌 | ⭐⭐⭐ | ❌ Too slow |
| Chakra UI | ~150KB | 🐌 | ⭐⭐⭐⭐ | ❌ Bloated |
| Ant Design | ~300KB | 🐌🐌 | ⭐⭐⭐ | ❌ Overkill |
| Pure Tailwind | ~10KB | ⚡⚡⚡⚡ | ⭐⭐ | ⚠️ Manual work |
...
**Result**: App loads in <500ms (Lighthouse 100 Performance) 🚀
